...
template <typename Dtype> const Dtype *BaseTensor<Dtype>::cpu_data() const {
  DCHECK(_memory);
...
Dtype BaseTensor<Dtype>::asum_data() const {
  ...
  case SyncedMemory::HEAD_AT_CPU:
    return chime_cpu_asum(_count, cpu_data());
...
template <typename Dtype> Dtype BaseTensor<Dtype>::asumq_data() const {
  Dtype sumsq;
  const Dtype *data;
  if (!_memory) {
    return 0;
...
template <typename Dtype> void BaseTensor<Dtype>::set_cpu_data(Dtype *data) {
  DCHECK(_memory);
  utens_t size = _count * sizeof(Dtype);
  if (_memory->size() != size) {
    _memory.reset(new SyncedMemory(size));
  }
...
template <typename Dtype> Dtype *BaseTensor<Dtype>::mutable_cpu_data() {
  DCHECK(_memory);
